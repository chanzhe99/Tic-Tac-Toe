...
private:
	...
	virtual void createRoomReturn(int localPlayerNr, const ExitGames::Common::Hashtable& gameProperties, const ExitGames::Common::Hashtable& playerProperties, int errorCode, const ExitGames::Common::JString& errorString) { std::wcout << "New room created." << std::endl; }
...
public:
	...
	static Photon& GetInstance(Game& CurrentGame)
	{
		static Photon Instance(CurrentGame);
		return Instance;
	}

	void SendMyID(int IDNum);
	void SendMyVector2Data(Vector2 Vector2Data);
	void SendReplayRequest(bool ReplayData);
	void SendString(const char* StringData, int StringSize);
};
